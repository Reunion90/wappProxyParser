...
$(document).ready(function () {
	...
	function fnSendFormData($oForm, $oSubmit) {
		...
		$oForm.find('input, select').each(function (iIndex, oElement) {
			var $oElement = $(oElement);

			if ($oElement.prop("tagName") == "INPUT") {
				if ($oElement.attr("type") == "checkbox") {
					if ($oElement.prop("checked")) aData[$oElement.attr("name")] = 1;
				} else {
					aData[$oElement.attr("name")] = $oElement.val();
				}
			}

			if ($oElement.prop("tagName") == "SELECT") aData[$oElement.attr("name")] = $oElement.find(":selected").val();
		});
	...
	function fnBindForms() {
		...
		$oForm.each(function (iIndex, oElement) {
			var $oElement = $(oElement);

			$oElement.addClass("binded");
			$oElement.find('button[type=submit]').click(function (oEvent) {
